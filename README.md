...
Препроцессинг заключался, в исключении значений NaN, +-inf из датасета, путем удаления столбцов (если количество NaN больше на 5%, чем строк тренировочного и тестового датасета), а также строк. P.S. Способ о сохранении всех строк (описан в коде), будет произведён позже есть ли останется время после докера!
